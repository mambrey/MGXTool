...
// Decision Bias Profile options with commentary - now as array for multi-select
const DECISION_BIAS_OPTIONS_LIST = [
  'Data Centric',
  'Margin Maximizer',
  'Price Leadership',
...
export default function ContactForm({ contact, accounts, onSave, onCancel }: ContactFormProps) {
  const [formData, setFormData] = useState({
    ...
    decisionBiasProfile: normalizeDecisionBiasProfile(contact?.decisionBiasProfile),
  ...
  // Decision Bias Profile multi-select handlers
  const handleDecisionBiasToggle = (bias: string) => {
    setFormData(prev => {
      const current = prev.decisionBiasProfile;
      const isSelected = current.includes(bias);
      
      if (isSelected) {
        return {
          ...prev,
          decisionBiasProfile: current.filter(b => b !== bias)
        };
      } else {
        return {
          ...prev,
          decisionBiasProfile: [...current, bias]
        };
      }
    });
  };

  const handleSelectAllDecisionBias = () => {
    const allSelected = formData.decisionBiasProfile.length === DECISION_BIAS_OPTIONS_LIST.length;
    
    if (allSelected) {
      setFormData(prev => ({ ...prev, decisionBiasProfile: [] }));
    } else {
      setFormData(prev => ({ ...prev, decisionBiasProfile: [...DECISION_BIAS_OPTIONS_LIST] }));
    }
  };

  const isAllDecisionBiasSelected = formData.decisionBiasProfile.length === DECISION_BIAS_OPTIONS_LIST.length;
            ...
            <div>
              ...
              <div className="mt-3 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                <p className="text-xs font-medium text-gray-700 mb-2">
                  Responsibility Level indicates how much authority the contact has for each area and selected product category.
                </p>
                <ul className="text-xs text-gray-600 space-y-1">
                  <li><span className="font-semibold">High</span> = final decision-maker and accountable for outcomes.</li>
                  <li><span className="font-semibold">Medium</span> = provides strong input and influences decisions, but does not have final approval.</li>
                  <li><span className="font-semibold">Low</span> = limited involvement, mainly informed or supports execution.</li>
                  <li><span className="font-semibold">None</span> = no involvement in this area.</li>
                </ul>
              </div>
            </div>
          </CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-base ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm birthday-no-year"
                />
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-2 p-4 border border-gray-200 rounded-lg space-y-3">
                <div className="flex items-center space-x-3 p-2 bg-blue-50 border border-blue-200 rounded">
                  <Checkbox
                    id="selectAllDecisionBias"
                    checked={isAllDecisionBiasSelected}
                    onCheckedChange={handleSelectAllDecisionBias}
                  />
                  <Label 
                    htmlFor="selectAllDecisionBias" 
                    className="font-semibold text-blue-900 cursor-pointer flex-1"
                  >
                    Select All
                  </Label>
                  <Badge variant="secondary" className="text-xs">
                    {formData.decisionBiasProfile.length} / {DECISION_BIAS_OPTIONS_LIST.length}
                  </Badge>
                </div>
                ...
                <ScrollArea className="h-[280px] pr-3">
                  <div className="space-y-2">
                    {DECISION_BIAS_OPTIONS_LIST.map((bias) => {
                      const biasInfo = DECISION_BIAS_OPTIONS[bias as keyof typeof DECISION_BIAS_OPTIONS];
                      return (
                        <div key={bias} className="flex items-start space-x-3 p-3 hover:bg-gray-50 rounded border border-gray-100">
                          <Checkbox
                            id={`bias-${bias}`}
                            checked={formData.decisionBiasProfile.includes(bias)}
                            onCheckedChange={() => handleDecisionBiasToggle(bias)}
                            className="mt-1"
                          />
                          <Label 
                            htmlFor={`bias-${bias}`} 
                          ...
                          >
                            ...
                            <div className="text-xs text-gray-600 mt-1">{biasInfo.description}</div>
                          </Label>
                        </div>
                      );
                    })}
              ...
              {formData.decisionBiasProfile.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {formData.decisionBiasProfile.map((bias) => (
                    <Badge key={bias} variant="secondary" className="flex items-center gap-1">
                      {bias}
                      <X 
                        className="w-3 h-3 cursor-pointer hover:text-red-500" 
                        onClick={() => handleDecisionBiasToggle(bias)}
                      />
                    </Badge>
                  ))}
                </div>
              ...
              {formData.decisionBiasProfile.length > 0 && (
                <Alert className="mt-3 bg-blue-50 border-blue-200">
                  ...
                  <AlertDescription className="text-sm">
                    <p className="text-blue-800 font-medium mb-2">Best to lead with:</p>
                    <ul className="list-disc list-inside space-y-1 text-blue-700">
                      {formData.decisionBiasProfile.map((bias) => {
                        const biasInfo = DECISION_BIAS_OPTIONS[bias as keyof typeof DECISION_BIAS_OPTIONS];
                        return (
                          <li key={bias}>
                            <span className="font-medium">{bias}:</span> {biasInfo.bestToLeadWith}
                          </li>
                        );
                      })}
                    </ul>
              ...
              <p className="text-xs text-gray-500 mt-2">
                Select one or more decision bias profiles that describe how this contact makes decisions
              </p>
            </div>
